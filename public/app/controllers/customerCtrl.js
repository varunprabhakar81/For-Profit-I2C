angular.module('customerController', ['customerServices'])

.controller('customerCtrl', function($http, $location, $timeout, $scope, Customer, GLAccount, Config, Chapter) {
    ...
    app.showCustomerEditModal = false;
    ...
    this.addCustomer = function(customerData, valid) {
        app.disabled = true;
        app.errorMsg = false;
        ...
        if (valid) {
            Customer.addCustomer(app.customerData).then(function(data) {
    ...
    // Function: get all the customers from database
    function getCustomers() {
        // Runs function to get all the customers from database
        Customer.getCustomers().then(function(data) {
            // Check if able to get data from database
            if (data.data.success) {
                // Check which permissions the logged in user has
                if (data.data.permission === 'admin' || data.data.permission === 'moderator') {
                    app.customers = data.data.customers; // Assign customers from database to variable
    ...
    function getChapters() {
        Chapter.getChapters().then(function(data) {
            ...
            if (data.data.success) {
                // Check which permissions the logged in user has
                if (data.data.permission === 'admin' || data.data.permission === 'moderator') {
                    app.chapters = data.data.chapters; // Assign customers from database to variable
                    app.loading = false; // Stop loading icon
                    app.accessDenied = false; // Show table
    ...
    getCustomers(); // Invoke function to get customers from databases
    getChapters();

    app.customerEditModal = function() {
    ...
    app.deleteCustomer = function(customername) {
        // Run function to delete a user
        Customer.deleteCustomer(customername).then(function(data) {
            // Check if able to delete user
            if (data.data.success) {
                getCustomers(); // Reset users on page
            } else {
...
.controller('editCustomerCtrl', function($scope, $routeParams, User, Customer, $timeout, $location, GLAccount, Config, Chapter) {
    ...
    function getCustomers() {
        Customer.getCustomer($routeParams.id).then(function(data) {
            // Check if the user's _id was found in database
            if (data.data.success) {
                $scope.newPrimaryInfo.customername = data.data.customer.customername;
                $scope.newPrimaryInfo.email = data.data.customer.email;
                $scope.newPrimaryInfo.aracct = data.data.customer.aracct;
                $scope.newPrimaryInfo.invoiceterms = data.data.customer.invoiceterms;
                $scope.newPrimaryInfo.chapters = data.data.customer.chapters;


                app.currentItem = data.data.customer._id; // Get user's _id for update functions
            } else {
                app.errorMsg = data.data.message; // Set error message
            }
        });        
    ...
    app.updatePrimaryInfo = function(newPrimaryInfo, valid) {
        ...
        if (valid) {
            var customerObject = {}; // Create a user object to pass to function
            customerObject._id = app.currentItem; // Get _id to search database
            customerObject.customername = newPrimaryInfo.customername;
            customerObject.email = newPrimaryInfo.email;
            customerObject.aracct = newPrimaryInfo.aracct;
            customerObject.invoiceterms = newPrimaryInfo.invoiceterms;
            customerObject.chapters = newPrimaryInfo.chapters;

            Customer.editCustomer(customerObject).then(function(data) {
